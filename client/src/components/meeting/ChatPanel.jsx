  ...
  return (
    <aside className="w-full sm:w-80 h-full bg-white border-1 border-slate-200 flex flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200">
          ...
          messages.map((msg, index) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`px-3.5 py-2.5 rounded-2xl max-w-[85%] text-sm loading-relaxed shadow-xs ${isMe ? "bg-primary text-white rounded-tr-none font-medium" : "bg-slate-100 text-slate-800 rounded-tl-none border border-slate-200 font-mediu"}`}
                >
                  {msg.text}
                </div>
              </div>
      ...
      <form
        ...
        className="p-3 border-t border-slate-200 bg-slate-50 flex items-cneter gap-2"
      >
        <input
          type="text"
